...
DimPlot(s1s2_scTransform, reduction = "umap", label = TRUE, pt.size = 0.5) + NoLegend()
...
plot = ggplot(data = filter(s1s2_scTransform.markers, !gene %in% list), aes(y=avg_logFC, x = cluster), color = "gray93", size = 1.5, shape = 16) +
  theme_classic() +
  geom_jitter(width = 0.2, shape = 16)+
  geom_jitter(data = filter(s1s2_scTransform.markersSubset, gene %in% list), aes(x=cluster, y=avg_logFC), color = "red", width = 0.2, shape = 16) +
  geom_text_repel(data = filter(s1s2_scTransform.markersSubset, gene %in% list), aes(x=cluster, y=avg_logFC, label = gene), color = "red") +
...
ggsave(plot, file = paste0(plotDirectory, 'FM01_seuratAllClusterGenesWBGN.svg'), width = 12, height = 6)
