########################################################################################
#Description: Analysis of clusters from DEGs for data from Herlyn Lab
...
dataDirectory <- "~/Dropbox (RajLab)/FateMap/REVISIONS/FateMapAnalysis_Maalavika/MHdata/seurat/"
dataDirectory1 <- "~/Dropbox (RajLab)/FateMap/REVISIONS/FateMapAnalysis_Maalavika/FM01_DEGandVarGenes/Data/"
plotDirectory <- "~/Dropbox (RajLab)/FateMap/REVISIONS/FateMapAnalysis_Maalavika/MHdata/seuratOutput/"

D0 <- Read10X_h5(paste0(dataDirectory, "D0_filtered_feature_bc_matrix.h5"))
dimnames(D0)[[1]] <- gsub("GRCh38_", "", dimnames(D0)[[1]])
D0 <- CreateSeuratObject(D0)
D0 <- D0[!grepl("mm10",substr(rownames(D0),1, 4)),]

EP <- Read10X_h5(paste0(dataDirectory, "EP_filtered_feature_bc_matrix.h5"))
dimnames(EP)[[1]] <- gsub("GRCh38_", "", dimnames(EP)[[1]])
EP <- CreateSeuratObject(EP)
EP <- EP[!grepl("mm10",substr(rownames(EP),1,4)),]

DEG_naive <- read.delim("~/Dropbox (RajLab)/FateMap/REVISIONS/FateMapAnalysis_Maalavika/")
DEG_resistant <- read.delim( "~/Dropbox (RajLab)/FateMap/REVISIONS/FateMapAnalysis_Maalavika/FM01_DEGandVarGenes/Data/CloneGenes_FC1_p0.05.tsv")
DEG_resistant <- unique(DEG_resistant$gene[order(DEG_resistant$avg_log2FC)])
...
saveRDS(df, paste0(dataDirectory, label,"_UMAP.rds"))
...
df <- readRDS(paste0(dataDirectory, label,"_UMAP.rds"))
...
ggsave(paste0(plotDirectory, "FeaturePlot1_",label, ".svg"), plot, width = 20, height = 20)
plot <- FeaturePlot(df,list[-c(1:15)])
ggsave(paste0(plotDirectory, "FeaturePlot2_",label, ".svg"), plot, width = 20, height = 15)
...
df <- readRDS(paste0(dataDirectory, label,"_UMAP.rds"))
df <- FindNeighbors(df)
df <- FindClusters(df, resolution = 0.6, verbose = FALSE)
df.markers <- FindAllMarkers(df, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.table(df.markers, paste0(dataDirectory, "MHdata_clusterDEG_",label,".tsv") , sep = "\t", quote = F)

df.markers <- read.delim(paste0(dataDirectory, "MHdata_clusterDEG_",label,".tsv"), sep = "\t")
...
venn.diagram(
  ...
  filename = paste0(plotDirectory, "ClusterDEG_CellLinevsPatient_",label,"_Venn.png" ),
  output=TRUE,
...
set.seed(100)
plot <- ggplot(fracOverlap, aes(x = name, y = value, col = sample))+
  geom_jitter( position=position_jitterdodge(dodge.width =0.8, jitter.width = 0.2), shape = 16) +
  labs(x="", y = "Fraction overlap")+
  theme_classic() 
ggsave(plot = plot,filename= paste0(plotDirectory, "MH_Random_real_overlap.svg"), height=10, width=10)
write.csv(fracOverlap, paste0(plotDirectory, "MH_Random_real_overlap.csv"))
