...
df <- as.data.frame(t(as.matrix(fm08@assays$integrated@scale.data)))
...
plot <- ggplot(clone_corr_mat, aes( x = Clonal_information, y = Correlation, color = Clonal_information)) + 
  ...
  rremove("legend") + 
  xlab("")
ggsave(paste0(plotDirectory,"FM08_CorrelationClonesvsRandom_allGenes.svg"),plot)
...
write.table(fc,paste0(plotDirectory,"FM08_CorrelationFC_allGenes.tsv"))
...
for (nVarGenes in c(50,100,500, 1000)) {
  #specify genes
  fm08 <- FindVariableFeatures(fm08, assay="RNA", nfeatures = nVarGenes)
  top10 <- head(VariableFeatures(fm08, assay="RNA"), 30)
  plot1 <- VariableFeaturePlot(fm08, assay="RNA")
  plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
  ggsave(paste0(plotDirectory,"FM08_VariableGenes_",nVarGenes,".svg"),plot2, width = 8, height= 10)
  
  VarGenes = VariableFeatures(fm08, assay="RNA")
  df= df_main
  ...
  plot <- ggplot(clone_corr_mat, aes( x = Clonal_information, y = Correlation, color = Clonal_information)) + 
    ...
    rremove("legend") + 
    xlab("")
  ggsave(paste0(plotDirectory,"FM08_CorrelationClonesvsRandom_VarGenes",nVarGenes,".svg"),plot, width =6, height = 5)
  ...
  fc <- group_by(fc, CloneSize) %>%
    summarize(mean(FoldChange))
  write.table(x = fc, file = paste0(plotDirectory,"FM08_CorrelationFC_VarGenes",nVarGenes,".tsv"),quote = F)
}
